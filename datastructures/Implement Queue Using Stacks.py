...
class MyQueue:
    ...
    def pop(self) -> int:
        # put everything from forward into backward (except for the last one which is returned)
        while len(self.forward) > 1:
            self.backward.append(self.forward.pop())

        last = self.forward.pop()

        # reverse everything from the backward back into forward
